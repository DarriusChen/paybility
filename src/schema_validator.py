...
def validate_data(file_type: str,
    ...
    try:
        ...
        logger.error(format_func_msg(func='validate_data',
                                     msg=f"讀取 schema 時發生錯誤: {e}"))
        return {
            "status": {"status": "False", "message": f"❌ 讀取 schema 失敗: {str(e)}"},
            "sub_status": {
                "entity_code": {"status": "False", "message": "❌ 無法驗證業者代碼"},
                "column_name": {"status": "False", "message": "❌ 無法驗證欄位名稱"}
            }
        }
    
    logger.info(format_func_msg(func='validate_data',
                                msg="開始驗證資料..."))
    ...
    if validation_passed:
        return {
            "status": {"status": "True", "message": "✅ Schema 驗證通過"},
            "sub_status": {
        ...
        return {
            "status": {"status": "False", "message": f"❌ Schema 驗證失敗: {'; '.join(error_messages)}"},
            "sub_status": {
                "entity_code": {"status": "True", "message": "✅ 業者代碼正確"},
                "column_name": {"status": "False", "message": f"❌ 欄位名稱不正確: {error_columns}"}
            }
        } 